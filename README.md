<h1 align="center">Hi there, I'm Royce Abiel 👋</h1>

<p align="center">
🎓 Undergraduate @ <strong>SLIIT Northern Uni</strong> &nbsp;|&nbsp; 💻 2nd Year, 2nd Semester <br>
🔧 Web Dev Learner (MERN & Django) &nbsp;|&nbsp; 🔐 Cyber Security Enthusiast
</p>

---
## 👨‍💻 About Me

I'm a second-year undergraduate passionate about tech, currently diving deep into **full-stack web development** using modern technologies.  

Although I have a growing curiosity for **Cyber Security**, my current focus is on building **real-world, dynamic web applications** using tools from the **MERN stack** and **Django** ecosystem.

🚧 This semester, I'm developing a project aimed at solving a real-world problem — sharpening both my **frontend** and **backend** skills.

---
## 🚀 Tech Stack

```txt
Languages:   C, C++, Java, HTML, CSS, JavaScript, PHP, MySQL  
Frameworks:  React.js, Express.js, Node.js, Django  
Databases:   MongoDB, MySQL  
Tools:       Git, GitHub, VS Code, Figma (basics)
```

## 🌱 Currently Learning

- 🔁 Git & GitHub (Version Control)  
- ⚛️ React.js (Frontend - MERN Stack)  
- 🐍 Django (Backend - Python Framework)  
- 🌐 Full-Stack Web Development  
- 💡 Building real-world, dynamic web apps

---

## 📚 Outside of Code

When I’m not coding or learning, you’ll find me:

- 📖 Reading to grow both personally & professionally  
- 🧠 Exploring new tech topics just out of curiosity  
- ✍️ Reflecting and planning future projects  

---

## 📫 Let's Connect

- [🔗 LinkedIn](https://www.linkedin.com/in/royceabiel)  
- [🐦 X (Twitter)](https://twitter.com/yourusername)  
- 📧 Email: [royceabiel26.uni@gmail.com](mailto:royceabiel26.uni@gmail.com)

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=CroosRRAF&show_icons=true&theme=radical" alt="CroosRRAF's GitHub Stats" width="45%" />
  &nbsp;
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=CroosRRAF&layout=compact&theme=radical" alt="Top Languages" width="45%" />
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=CroosRRAF&label=Profile%20Views&color=blueviolet&style=flat" alt="Profile Views" />
</p>

<p align="center"><em>"The expert in anything was once a beginner."</em></p>
